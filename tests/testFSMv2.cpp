...
namespace {
    ...
    class BaseState {
    public:
        explicit BaseState(std::reference_wrapper<FSMv2> context) : _context(context) {
        }

    protected:
        std::reference_wrapper<FSMv2> _context;
        friend struct TransitionTable;
    };

    class LockedState : public BaseState {
    public:
        explicit LockedState(std::reference_wrapper<FSMv2> context);
        friend struct TransitionTable;
    };

    class PaymentProcessingState : public BaseState {
    public:
        explicit PaymentProcessingState(std::reference_wrapper<FSMv2> context, std::string cardNumber);
    ...
    public:
        PaymentFailed(std::reference_wrapper<FSMv2> context, std::string reason);
    ...
    public:
        PaymentSuccess(std::reference_wrapper<FSMv2> context, int fare, int balance);
    ...
    public:
        explicit Unlocked(std::reference_wrapper<FSMv2> context);
        friend struct TransitionTable;
    };
    ...
    class FSMv2 {
    public:
        FSMv2() : _fsm{LockedState{std::ref(*this)}} {
        }

        template <typename Event>
        FSMv2 & process(Event event) {
    ...
    private:
        ...
        POSTerminal _pos{""};
        LEDController _led;
        FSM<TransitionTable, LockedState, PaymentProcessingState, PaymentFailed, PaymentSuccess, Unlocked> _fsm;
    ...
    void FSMv2::initiateTransaction(const std::string & gateway, const std::string & cardNum, int amount) {
    ...
    PaymentProcessingState::PaymentProcessingState(std::reference_wrapper<FSMv2> context, std::string cardNumber)
        : BaseState(context)
    ...
    PaymentFailed::PaymentFailed(std::reference_wrapper<FSMv2> context, std::string reason)
        : BaseState(context)
    ...
    PaymentSuccess::PaymentSuccess(std::reference_wrapper<FSMv2> context, int fare, int balance)
        : BaseState(context)
        , _timeoutManager(
              [&] {
                  _context.get().process(Timeout{});
    ...
    Unlocked::Unlocked(std::reference_wrapper<FSMv2> context) : BaseState(context) {
        auto & fsm = _context.get();
        fsm.getDoor().open();
        fsm.getLED().setStatus(LEDController::eStatus::GreenArrow);
...
TEST(FSMv2, TestPaymentProcessing) {
    FSMv2 fsm;
...
TEST(FSMv2, TestTimeoutOnPaymentProcessing) {
    FSMv2 fsm;
...
TEST(FSMv2, TestLockedFromPaymentFailed) {
    FSMv2 fsm;
...
TEST(FSMv2, TestPaymentSuccessful) {
    FSMv2 fsm;
...
TEST(FSMv2, TestLockedFromUnlocked) {
    FSMv2 fsm;
...
TEST(FSMv2, TestLockedFromPaymentSuccessful) {
    FSMv2 fsm;
